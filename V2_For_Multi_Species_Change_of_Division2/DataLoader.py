...
class DataLoader:
    ...
    def distribute_data(self, url, test_num):
        ...
        x_num = math.ceil((x2 - x1) / width)
        y_num = math.ceil((y2 - y1) / width)
        x_degree = width
        y_degree = width
        ...
        for i in range(category_num):
            ...
            elif len(one) < self.info.interactive_threshold:  # number of interactions less than the threshold
                # category_num -= 1
                discard_num += 1
        ...
        print("After clear---category num: " + str(category_num - discard_num) + " ; grid num: " + str(
            x_num * y_num) + " ; test shop num:" + str(test_set_num) + "---")
